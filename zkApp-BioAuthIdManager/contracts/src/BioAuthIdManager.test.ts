import { BioAuthIdManager } from './BioAuthIdManager';
import {
  isReady,
  shutdown,
  Field,
...
/*
 * This file specifies how to test the `BioAuthIdManager` example smart contract. It is safe to delete this file and replace
 * with your own tests.
 *
...
describe('BioAuthIdManager', () => {
  jest.setTimeout(1000 * 100);

  let deployerAccount: PrivateKey,
    zkAppAddress: PublicKey,
    zkAppPrivateKey: PrivateKey,
    zkApp: BioAuthIdManager;
  let Local: any;

  beforeAll(async () => {
    await isReady;
    if (proofsEnabled) BioAuthIdManager.compile();
  });
  ...
  beforeEach(() => {
    ...
    zkApp = new BioAuthIdManager(zkAppAddress);
  });
  ...
  it('generates and deploys the `BioAuthIdManager` smart contract', async () => {
    await localDeploy();
    const num = zkApp.num.get();
    expect(num).toEqual(Field(1));
  });

  it('correctly updates the num state on the `BioAuthIdManager` smart contract', async () => {
    await localDeploy();
  ...
  it('correctly updates the numBioAuthed state on the `BioAuthIdManager` smart contract', async () => {
    await localDeploy();
